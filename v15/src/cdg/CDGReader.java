...
public class CDGReader extends TimerTask {
	...
	int get_framecounter(){
		return frame_counter;
	}
	void set_framecounter(int value){
		frame_counter1=value;
		if(frame_counter1<0)
			wait=Math.abs(frame_counter1)*50;
	}
	
	public CDGReader(File cdgFile, CDGViewer viewer) {
		this.viewer=viewer;
		frame_counter=0;
		try {
	...
	public void process() {
		System.out.print(FrameInterval);
		timer.scheduleAtFixedRate(this, wait, FrameInterval);
	}
	...
	public void run() {
		...
		while(frame_counter<frame_counter1) {
			if(frame_counter1<0) {
				try {
					TimeUnit.MILLISECONDS.sleep(Math.abs(frame_counter1)*50);
